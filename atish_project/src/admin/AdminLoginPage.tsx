...
const AdminLoginPage = () => {
  ...
  // Once signed in, redirect based on role.
  useEffect(() => {
    if (auth.user) {
    ...
    <div className="min-h-screen flex flex-col items-center justify-center py-10 px-4 bg-[#D5D3C8] relative">
      ...
      <div className="relative w-full max-w-md bg-white rounded-[2rem] shadow-card p-8 sm:p-10 border border-gray-100 mb-8 z-10">
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          {auth.error ? <div className="text-xs text-red-500 font-medium text-center">{auth.error}</div> : null}
          
          <Button type="submit" className="w-full mt-2" disabled={auth.loading}>
            {auth.loading ? "Signing in..." : "Sign in with Email"}
          </Button>
        </form>
        ...
        <div className="mt-6">
          ...
          <div className="text-center text-sm text-gray-500 mt-4">
            Don't have an account? <Link to="/admin/register" className="text-black font-semibold hover:underline">Sign up</Link>
          </div>
        </div>
